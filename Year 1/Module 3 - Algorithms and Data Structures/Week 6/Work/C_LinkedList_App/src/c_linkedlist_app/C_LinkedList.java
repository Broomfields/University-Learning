...
public class C_LinkedList
{
    ...
    public C_LinkedList(int[] a)
    {
        ...
        {
            newNode = new ListNode(a[size - index], prev);
            if (index == 1)
            {
                last = newNode;
            }
    ...
    public String toString()
    {
        ...
        while (current != head)
        {
            stringBuilder.append(current.data).append(" "); // append the data stored in the current node
            current = current.next; // move to the next node
        }

        return stringBuilder.toString().trim(); // return the string representation of the list
    }
    ...
    public boolean delete(int n)
    {
        ...
        for (int iota = 1; iota < n; iota++)
        {
            prev = current;
            current = current.next; // move to the next node
            if (current == head)
            { // if we reach the dummy node before finding the nth node
                return false;
            }
        }
